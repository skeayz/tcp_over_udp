...
class sendfile:
    ...
    def receive(self):
        ...
        while ack != self.final_ack:
            time_window.append((datetime.datetime.now().time() - start, self.window_print))
            try:
                ...
                if(ack >= self.lastAck):
                    if (self.ss_tresh > self.window_size):
                        ...
                        with self.lock:
                            ...
                            self.ss_tresh = (self.window_size + window_incr) if (self.window_size + window_incr) < 20 else 20
                    with self.lock:
        ...
        print(datetime.datetime.now().time() - start, self.window_print)
